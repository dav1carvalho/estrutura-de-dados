...
unsigned Hash::Buscar(const std::string &chave) const
{
    ...
    bool armazenada = false;
    unsigned cont = 0;

    while (armazenada == false and cont < mCapacidade)
    {
    ...
    {
        throw invalid_argument("chave nao armazenada!");
    }
}
...
void Hash::Inserir(const string &chave, const TValor &valor)
{
    ...
    if (mTamanho < mCapacidade)
    {
        ...
        {
            cout << "essa chave já existe!" << endl;
        }
    }
    else
    {
