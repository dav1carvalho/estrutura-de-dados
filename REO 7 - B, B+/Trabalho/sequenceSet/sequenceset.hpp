...
class sequenceset {
    private:
        ...
        dado buscaBinaria(dado vetor[], int inicio, int fim, tipoChave chave, int &contador);       
    public:
        ...
        void imprimir();
        void depurar();
        dado buscar(tipoChave chave, int &contador);
        /* 
...
dado sequenceset::buscar(tipoChave chave, int &contador) {
    if (posPrimeiroPacote == POS_INVALIDA) {
    ...
    // procura o pacote que poderia conter o elemento
        ...
        return buscaBinaria(umPacote->elementos, 0, umPacote->numElementos-1, chave, contador);
    } 
}

//funcao de busca binaria recursiva
dado sequenceset::buscaBinaria(dado vetor[], int inicio, int fim, tipoChave chave, int &contador) {
    int meio = (inicio+fim)/2;
    contador++;
    if (inicio <= fim) {
        if (strcmp(chave, vetor[meio].nome) == 0)
            return vetor[meio];
        else if (strcmp(chave, vetor[meio].nome) > 0)
            return buscaBinaria(vetor,meio+1,fim,chave, contador);
        else
            return buscaBinaria(vetor,inicio,meio-1,chave, contador);
